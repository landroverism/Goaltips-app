...
import { fetchFixturesByDate } from '../../services/api';
...
const UpcomingMatchesComponent: React.FC = () => {
  ...
  const loadMatches = async (date: Date) => {
    try {
      ...
      const response = await fetchFixturesByDate(formattedDate);
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* You can add a date picker component here */}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center p-8 border rounded-lg bg-muted/20">
          <p className="text-muted-foreground">No upcoming matches for this date</p>
        </div>
      ) : (
